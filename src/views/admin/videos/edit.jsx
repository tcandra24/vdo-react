...
const Create = () => {
  ...
  const getDetailVideo = async () => {
    dispatchVideo({ type: "GET_VIDEO" });

    try {
      api.defaults.headers.common["Authorization"] = token;
      const { data } = await api.get(`/api/videos/${id}`);

      dispatchVideo({ type: "GET_VIDEO_SUCCESS" });

      name.setValue(data.data.name);
      videoId.setValue(data.data.video_id);
      categoryId.setValue(data.data.category.id);
    } catch (error) {
  ...
  const submit = async (e) => {
    dispatchVideo({ type: "UPDATE_VIDEOS" });
    ...
    try {
      ...
      dispatchVideo({ type: "UPDATE_VIDEOS_SUCCESS" });
